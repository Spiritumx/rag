...
# INNOVATION 1: Import v2 unified retriever
from unified_retriever_v2 import UnifiedRetriever
...
if dense_source:
    ...
    print(f"[Retriever V2] Loaded dense model from {dense_source}")
else:
    print("[Retriever V2] Dense model not provided. HNSW retrieval disabled.")

if splade_source:
    splade_model, splade_tokenizer = _load_splade_components(splade_source)
    print(f"[Retriever V2] Loaded SPLADE model from {splade_source}")
else:
    print("[Retriever V2] SPLADE model not provided. SPLADE retrieval disabled.")

if reranker_source:
    reranker_model = _load_reranker(reranker_source, device=device_override)
    if reranker_model is not None:
        print(f"[Retriever V2] Loaded reranker model from {reranker_source}")
    else:
        print(f"[Retriever V2] Failed to load reranker from {reranker_source}")
else:
    print("[Retriever V2] Reranker model not provided. Hybrid ranking will rely on ES scores only.")
...
@app.get("/")
async def index():
    return {"message": "Hello! This is the Innovation V2 Adaptive Retrieval Server."}
...
async def retrieve(request: Request):  # see the corresponding method in unified_retriever.py
    try:
        ...
        print(f"[Retriever V2] Corpus: {corpus_name} | Backend: {retrieval_backend} | Query: {query_text[:80]}...")
        ...
        try:
            ...
            print(f"[Retriever V2] Retrieved {len(retrieval)} docs in {time_in_seconds}s from {corpus_name}")
...
if __name__ == "__main__":
    import uvicorn
    # INNOVATION 1: Port 8002 for V2 retriever server
    port = int(os.environ.get("RETRIEVER_PORT", "8002"))
    print("="*80)
    print(f"[Innovation V2] Starting Adaptive Retrieval Server on port {port}")
    print("="*80)
    print(f"[Innovation V2] Features:")
    print("  - Query Analyzer: NER + Semantic Classifier")
    print("  - Dynamic Weights: Entity-dense vs Abstract queries")
    print("  - Baseline compatibility: Port 8001 unchanged")
    print("="*80)
