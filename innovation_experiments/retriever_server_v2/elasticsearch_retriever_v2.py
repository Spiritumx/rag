...
# INNOVATION 1: Import QueryAnalyzer
from query_analyzer import QueryAnalyzer


class ElasticsearchRetriever:
    """
    V2 Elasticsearch Retriever with Adaptive Retrieval (Innovation 1).

    Integrates QueryAnalyzer to dynamically adjust hybrid retrieval weights
    based on query characteristics (entity density + semantic abstractness).
    ...
    def __init__(
        ...
        # INNOVATION 1: Initialize QueryAnalyzer for adaptive retrieval
        self.query_analyzer = QueryAnalyzer(device=str(self.device))
    ...
    def retrieve_paragraphs(
        ...
        if retrieval_backend == "bm25":
            ...
            # INNOVATION 1: Adaptive Hybrid Retrieval
            if hybrid_weights is None:
                # Use QueryAnalyzer to compute dynamic weights
                analyzer_result = self.query_analyzer.get_dynamic_weights(query_text)
                weights = {
                    "bm25": analyzer_result['bm25'],
                    "splade": analyzer_result['splade'],
                    "dense": analyzer_result['dense'],
                }
                print(f"[Adaptive V2] Strategy: {analyzer_result['strategy']}")
                print(f"[Adaptive V2] Weights: BM25={weights['bm25']:.2f}, SPLADE={weights['splade']:.2f}, Dense={weights['dense']:.2f}")
            else:
                # Explicit weights override adaptive computation
                weights = self.hybrid_weights.copy()
                weights.update(hybrid_weights)
                print(f"[Adaptive V2] Using explicit weights: {weights}")
...
if __name__ == "__main__":
    ...
    print("\n\nRetrieving Titles ...")
    results = retriever.retrieve_titles("injuries", corpus_name=args.dataset_name)
    for result in results:
        print(result)

    print("\n\nRetrieving Paragraphs ...")
    results = retriever.retrieve_paragraphs(corpus_name=args.dataset_name, query_text="injuries")
    for result in results:
        print(result)
