"""
Result management for V2 pipeline with cascade analysis support.

Extends baseline ResultManager with:
- V2 output paths (separate from baseline)
- Cascade log management
- Stage 1 shared with baseline (for fair comparison)
"""
...
class ResultManagerV2:
    """Manage saving/loading of V2 results with cascade support."""

    def __init__(self, config):
        self.config = config

        # Stage 1 is SHARED with baseline for fair comparison
        self.stage1_dir = config['outputs']['stage1_dir']

        # Stage 2 and 3 use V2 directories
        self.stage2_dir = config['outputs']['stage2_dir']
        self.stage3_dir = config['outputs']['stage3_dir']

        # Cascade analysis directory (V2-specific)
        self.cascade_dir = config['outputs']['cascade_dir']
    ...
    def get_stage1_output_path(self, dataset_name: str) -> str:
        ...
        NOTE: Stage 1 is SHARED with baseline for fair comparison.
        """
    ...
    def get_stage2_output_path(self, dataset_name: str) -> str:
        """Get path for stage 2 prediction results (V2 directory)."""
        return os.path.join(self.stage2_dir, f'{dataset_name}_predictions_v2.json')

    def get_stage3_output_path(self) -> str:
        """Get path for stage 3 metrics (V2 directory)."""
        return os.path.join(self.stage3_dir, 'overall_metrics_v2.json')

    def get_cascade_log_path(self, dataset_name: str) -> str:
        """Get path for cascade routing log."""
        return os.path.join(self.cascade_dir, f'{dataset_name}_cascade_log.csv')

    def get_cascade_analysis_path(self) -> str:
        """Get path for cascade analysis results."""
        return os.path.join(self.cascade_dir, 'cascade_analysis.json')
    ...
    def load_stage1_results(self, dataset_name: str) -> Dict:
        ...
        NOTE: Loads from SHARED baseline directory for fair comparison.
        """
    ...
    def load_stage2_results(self, dataset_name: str) -> Dict:
        """Load V2 prediction results for a dataset."""
